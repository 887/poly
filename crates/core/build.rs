...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Bake locale FTL content DIRECTLY into per-locale generated files (not
    // via an indirect include_str!). Each locale gets its own file so that
    // touching `locales/en/main.ftl` only dirtys `baked_locales_en.rs` —
    // `baked_locales_de.rs` etc. are unchanged, reducing incremental rebuild
    // scope and PR-conflict surface on the generated files.
    let workspace_root = crate_root.join("..").join("..");
    let locales_dir = workspace_root.join("locales");
    let i18n_dir = crate_root.join("src").join("i18n");
    // Ensure the output directory exists (it holds only generated files).
    std::fs::create_dir_all(&i18n_dir)
        .map_err(|e| format!("build.rs: create src/i18n/: {e}"))?;
    for locale in ["en", "de", "es", "fr"] {
        ...
        let baked_path = i18n_dir.join(format!("baked_locales_{locale}.rs"));
        let prev = std::fs::read_to_string(&baked_path).unwrap_or_default();
        if prev != baked {
            std::fs::write(&baked_path, &baked)
                .map_err(|e| format!("build.rs: write baked_locales_{locale}.rs: {e}"))?;
        }
    }
