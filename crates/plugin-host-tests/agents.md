...
├── src/lib.rs              # Shared test helpers (workspace_root, wasm_dir, load_plugin)
└── tests/
    ...
    └── client_e2e/
        ...
        ├── stoat.rs        # Stoat: 10 stub behavior verification tests
...
| `test-stoat` | No | Stub verification: correct types, error returns, empty lists |
...
### Stub Client Tests (10 tests each × 5 clients = 50 tests)

Each stub (stoat, matrix, discord, teams, server) verifies:
...
- **Lifecycle**: `authenticate_with_token()`, `authenticate_returns_error()`, `logout_succeeds()`
