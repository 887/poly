...
/// Generate demo voice channel participants.
///
/// Returns a mapping of channel ID → list of voice participants,
/// simulating users currently in voice channels.
pub fn demo_voice_participants() -> std::collections::HashMap<String, Vec<VoiceParticipant>> {
    let users = demo_users();
    let mut map = std::collections::HashMap::new();

    // Dev Voice: Alice (speaking, unmuted) + Bob (muted)
    map.insert(
        "ch-voice-dev".to_string(),
        vec![
            VoiceParticipant {
    ...
    // Gaming Voice: Grace (streaming screen) + Henry (video on) + Jack
    map.insert(
        "ch-voice-gaming".to_string(),
        vec![
            VoiceParticipant {
