...
use super::voice_common::{
    TransmitMode, VortexServerInfo, StoatVoiceError,
    OPUS_FRAME_SAMPLES, OPUS_MAX_DECODE_SAMPLES,
};

// ── Constants (native-only) ───────────────────────────────────────────────────

/// Opus application mode for voice.
/// NATIVE ONLY — uses `audiopus::Application` which doesn't compile on wasm32.
