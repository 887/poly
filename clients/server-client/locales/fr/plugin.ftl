...
plugin-poly-signup-description = Entrez l'URL de votre serveur pour vous connecter. Si votre clé est déjà enregistrée, vous serez connecté automatiquement.
...
plugin-poly-signup-no-account-desc = Aucun compte n'a été trouvé pour la clé de votre appareil sur ce serveur. Choisissez un nom d'utilisateur pour créer un nouveau compte.
...
plugin-poly-signup-no-identity = Aucune clé d'identité trouvée. Créez-en une dans Paramètres → Identité.
