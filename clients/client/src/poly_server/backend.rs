...
//!
//! Bridges the poly-server HTTP + WS clients into the [`ClientBackend`](crate::ClientBackend)
...
use super::http::{PolyServerConfig, PolyServerHttpClient};
use super::models::{self as srv, ChannelKind, FriendRequestStatus};
use super::ws::PolyServerWsClient;
use crate::*;
...
impl ClientBackend for PolyServerBackend {
    ...
    async fn get_friends(&self) -> ClientResult<Vec<User>> {
        ...
        let my_id = self.account_id.clone().unwrap_or_default();
        let mut friends = Vec::new();
        for req in &requests {
            if req.status == FriendRequestStatus::Accepted {
                let friend_id = if req.from == my_id {
                    &req.to
                } else {
                    &req.from
                };
                if let Ok(user) = self.get_user(friend_id).await {
                    friends.push(user);
                }
            }
        }
