//! `ServerAdminBackend` + `WritableServerAdminBackend` for `DiscordClient`.
//!
//! Tier 2 (plan-trait-split-readable-vs-writable): only
...
impl poly_client::ServerAdminBackend for DiscordClient {
    async fn mark_channel_read(&self, _channel_id: &str) -> ClientResult<()> {
        Err(ClientError::NotSupported(
            "discord: mark_channel_read not implemented".to_string(),
        ))
    }
    ...
    async fn respond_to_server_invite(
        ...
        Err(ClientError::NotSupported(
            "discord: respond_to_server_invite not implemented".to_string(),
        ))
    }
    ...
    async fn invite_user_to_server(
        ...
        let system_channel_id = guild.system_channel_id.ok_or_else(|| {
            ClientError::NotSupported(
                "invite_user_to_server: server has no system channel; cannot create invite"
                    .to_string(),
            )
        })?;
...
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl poly_client::WritableServerAdminBackend for DiscordClient {
    async fn create_server(&self, _name: &str) -> ClientResult<Server> {
        Err(ClientError::NotSupported(
            "discord: create_server not implemented".to_string(),
        ))
    }
    ...
    async fn create_channel(
        ...
        Err(ClientError::NotSupported(
            "discord: create_channel not implemented".to_string(),
        ))
    }
    ...
    async fn update_server_banner(
        ...
        self.http.patch_guild(server_id, body).await.map(|_| ())
    }
}
