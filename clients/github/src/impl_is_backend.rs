...
impl IsBackend for GitHubClient {
    ...
    ///
    /// Capability dispatch: GitHub supports writing only to single-issue
    /// thread channels (`gh-issue-{owner}~{repo}-{number}`).  The
    /// per-channel-kind routing lives in
    /// [`impl WritableMessagingBackend`](impl_writable_messaging.rs).
