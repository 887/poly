...
fn make_story(id: u64, title: &str, url: Option<&str>, score: u32, descendants: u32, kids: Vec<u64>) -> Value {
    let mut story = json!({
        "id": id,
        "type": "story",
        "by": "testuser",
        "time": 1700000000u64 + id,
        "title": title,
        "score": score,
...
fn make_ask_story(id: u64, title: &str, score: u32, descendants: u32, kids: Vec<u64>) -> Value {
    json!({
        "id": id,
        "type": "story",
        "by": "askuser",
...
fn make_comment(id: u64, parent: u64, author: &str, text: &str, kids: Vec<u64>) -> Value {
    json!({
        "id": id,
        "type": "comment",
        "by": author,
        "time": 1700000100u64 + id,
        "text": text,
        "parent": parent,
        "kids": kids
    })
...
fn item_for_id(id: u64) -> Option<Value> {
    match id {
        ...
        100 => Some(make_comment(100, 1, "rustlover", "Great language, very fast compile times.", vec![113])),
        101 => Some(make_comment(101, 1, "skeptic99", "Until you get to fighting the borrow checker...", vec![])),
        102 => Some(make_comment(102, 1, "newbie_coder", "Is it good for beginners?", vec![])),
        103 => Some(make_comment(103, 2, "aiwatch", "Context windows keep growing. When does it stop?", vec![])),
        104 => Some(make_comment(104, 2, "ml_eng", "Running this in prod. Very impressive for long documents.", vec![])),
        105 => Some(make_comment(105, 3, "emailpurist", "HTML email is the bane of my existence.", vec![])),
        106 => Some(make_comment(106, 5, "dba_guru", "The new logical replication features are great.", vec![])),
        107 => Some(make_comment(107, 5, "pg_fan", "Postgres just keeps getting better every year.", vec![])),
        108 => Some(make_comment(108, 7, "wasm_dev", "Dioxus is one of the most ergonomic WASM frameworks I've used.", vec![])),
        109 => Some(make_comment(109, 9, "cli_wizard", "eza (modern ls replacement) and bat (modern cat).", vec![])),
        110 => Some(make_comment(110, 9, "poweruser42", "fzf, zoxide, and ripgrep are my holy trinity.", vec![])),
        111 => Some(make_comment(111, 10, "dotfile_mgr", "I use chezmoi with a private git repo. Works great.", vec![])),
        112 => Some(make_comment(112, 11, "keyboard_nerd", "Which corpus did you use for the optimization?", vec![])),
        113 => Some(make_comment(113, 100, "rustlover", "Though with incremental compilation it's gotten much better.", vec![])),
        _ => None,
    }
}
