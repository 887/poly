...
| P68 | 🟠 | **Regional-indicator flag emojis render as black** (e.g. 🇩🇪 on the Berlin demo message). Linux + Chromium without Noto Color Emoji falls back to a generic flag glyph. Ship a `font-family` fallback chain (Apple / Noto / Segoe / Twemoji). Longer term: document Noto Color Emoji as a recommended system dep in the README. | `crates/core/assets/styling/layout.css` body font-family |
| P69 | 🟡 | **UTF-8 em-dash mojibake in `CustomBlock`.** `—` renders as `â ` in the forum-detail inline pane path. The same chat-view markdown→ammonia→dangerous_inner_html path also suspect. Root cause: somewhere between WIT `string` round-trip and Dioxus DOM insertion, UTF-8 bytes are being interpreted as Latin1. Needs deeper tracing (bindgen? ammonia? pulldown_cmark?). Deferred — now moot on forum detail because WP J's route-push change means forum posts render via the existing `ForumPostView` (not `CustomBlock`). But chat-view + any future CustomBlock usage still affected. | `crates/core/src/ui/client_ui/custom_block.rs`, `chat_view.rs` markdown path |
