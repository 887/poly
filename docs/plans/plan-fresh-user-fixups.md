...
- [x] **U.6** ✅ Added an overflow (⋯) menu to the mobile chat header (`layout.rs` render_mobile_chat_header_right_toggle, reusing ctx.header_actions_menu_open — no new hook) with Search / Threads / Pinned / Settings / Agent. Each item opens its panel AND the right wing (ui_layout + MOBILE_RIGHT_WING_OPEN_JS), so the panel shows full-screen; the open wing also exposes a tool-row for switching. Live-confirmed on 390px: ⋯ → 5 items → Search opens full-screen (wing x:39 w:351). The chat header buttons drop from 8 to 3 — verify
