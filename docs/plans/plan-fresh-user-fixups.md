...
- [ ] **U.6** [SCOPED — real gap, high blast radius] **Root cause (confirmed 2026-06-04):** mobile uses a SEPARATE header render (`chat_view/layout.rs:335`, `.chat-header-actions-mobile`) that emits only Call / Video / right-wing-toggle (members) — NOT the desktop `.chat-header-actions-primary` + overflow path. So Settings / Threads / Pinned / Search / Agent have **no mobile entry point at all** (the right-wing toggle opens members, not the tools). FIX (deliberate): add an overflow ⋯ to the mobile header opening those utility panels, and render utility/agent panels full-screen on mobile. Header is on every chat → land with care + full reverify. The chat header buttons drop from 8 to 3 — verify
