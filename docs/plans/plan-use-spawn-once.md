...
### Phase 1 — Introduce the hook, no call-site changes

**Deliverable:** `crates/core/src/state/use_spawn_once.rs` (~150 LoC incl. tests).

Tasks:
- [ ] Create `crates/core/src/state/use_spawn_once.rs` with the `use_spawn_once` hook.
- [ ] Unit tests covering:
  - [ ] Same `key` → exactly one spawn.
  - [ ] Key change → new spawn (previous task continues; we don't cancel).
  - [ ] Drop while spawned future is in-flight → future continues (caller's problem).
  - [ ] Interaction with `BatchedSignal` mutations inside the spawned future.
- [ ] Re-export from `crates/core/src/state/mod.rs`.
- [ ] Document the hook in `docs/dev/reactive-state.md` (or create if missing) alongside `BatchedSignal`.
...
### Phase 2 — Migrate the 2 known correct sites

Migrate the sites that already have the hand-rolled pattern, so they demonstrate the new hook and we have a baseline:

- [ ] `crates/core/src/ui/routes.rs` `ServerChat` (L1681-region `spawned_for` + `use_effect` → `use_spawn_once`).
- [ ] `crates/core/src/ui/routes.rs` `ServerHome` (just-shipped `spawned_for` from commit `904920b9` → `use_spawn_once`).

Each migration should shrink ~20 lines to ~3 lines. Net diff should be clearly smaller.

### Phase 3 — Migrate HIGH-severity sites from audit
...
- [ ] `crates/core/src/ui/routes.rs :: ServerMediaViewerRoute` L1475 — effect reads `chat_data.{current_server,current_channel,channels}`, spawns `restore_server_channel(chat_data, ...)` which writes those same fields. Only an `already_loaded` guard, no `spawned_for`. If the URL channel doesn't exist on the server, `already_loaded` never flips true and the spawn restarts every time another `chat_data` write lands. Migration key: the URL `channel_id`. Pattern: copy from `ServerChat::use_effect` (same file, L1694, already correct).

- [ ] `crates/core/src/ui/account/common/forum_view.rs :: ForumPostView` L229 — same shape as above: reads `chat_data.{current_channel,current_server}`, spawns `restore_server_channel` that writes those fields, only `already_loaded` guard. Same failure mode on a stale forum-post deep link. Migration key: URL `channel_id` + `server_id` tuple.
...
### Phase 4 — Migrate MEDIUM-severity sites opportunistically

Effects that aren't a direct-cycle bug today but are fragile — one signal shape change away from wedging:

- [ ] `crates/core/src/ui/routes.rs :: DmChat` L1259 — pending-direct-call dispatch. Relies on `.take()` consuming the option as an implicit guard. Explicit key (`pending.account_id + dm_id`) is clearer.
- [ ] `crates/core/src/ui/account/common/chat_view.rs :: use_search_effect` L1487 — search-on-keystroke. No cycle (writes different signal), but no debounce either — each keystroke re-spawns. Key should be `(query, channel_id)`; combine with a debounce primitive separately.
- [ ] `crates/core/src/ui/account/common/chat_view.rs :: use_pinned_messages_effect` L1532 — acceptable today, flip for uniformity when touched.
- [ ] `crates/core/src/ui/account/common/chat_view.rs :: use_command_preload_effect` L1652 — spawns on every channel switch; no guard. Key: `channel_id`.
- [ ] `crates/core/src/ui/account/common/chat_view.rs :: use_member_list_effect` L1420 — similar shape; key: `active_channel_id`.
- [ ] `crates/core/src/ui/account/common/thread_view.rs :: ThreadPanel` L289 — reads `thread_id` + `active_account_id`, spawns `get_messages`. Safe for single-thread open, fragile if ever opens concurrently. Key: `(thread_id, account_id)`.
...
### Phase 5 — Clippy / dylint ban on raw pattern

Two tracks, ship whichever is ready first:

**Track A (fast, regex CI check):**
- [ ] Add `tools/scripts/forbid-use-effect-spawn-cycle.sh` — scans `crates/core/src/ui/**/*.rs` for `use_effect(move || { ... spawn(async move { ... signal.batch|write|set ... }) })` patterns.
- [ ] Allowlist `tools/scripts/use-effect-spawn-cycle-allowlist.txt` for intentional cases (debounced effects, multi-key scenarios).
- [ ] Wire into CI.

**Track B (proper, dylint):**
- [ ] Custom lint `tools/lints/poly-lints/src/use_effect_spawn_cycle.rs` matching the AST pattern.
- [ ] Exception annotation: `#[allow(poly::use_effect_spawn_cycle)]` with a required rationale comment.

### Phase 6 — Documentation + cleanup

- [ ] Update `CLAUDE.md` §"Common WASM-hang causes" #3 to cite `use_spawn_once` as the prescribed prevention.
- [ ] Add section to `docs/dev/reactive-state.md` with canonical patterns (keyed async load, debounced sync effect, non-spawning reactive effect).
- [ ] Remove any remaining `spawned_for: Signal<Option<K>>` hand-rolled bindings that the lint flagged as migrations-in-progress.
