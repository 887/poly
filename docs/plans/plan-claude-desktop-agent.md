...
- [x] **C.4** SSE transport — **shipped 2026-05-02.** Both endpoints now serve unconditionally: `POST /mcp` (polling, the existing JSON-RPC route) and `GET /mcp/sse` (server-sent events streaming `notifications/event` frames from the same `EventStore` broadcast channel that backs `poll_events`). Implementation: `mcp/chat-mcp/src/main.rs::handle_mcp_sse` using `axum::response::sse::Sse` + `tokio_stream::wrappers::BroadcastStream`. UI exposes a transport toggle in the agent Integrations page (KV `agent.mcp.transport`, default `"polling"`); the toggle changes which URL the recommended config snippet shows, with a tooltip linking [anthropics/claude-code#4118](https://github.com/anthropics/claude-code/issues/4118) (closed-as-duplicate of #13646) explaining when to flip it on. When Anthropic ships the fix, users flip the toggle — no Poly-side change required.
