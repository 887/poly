...
> **Created:** 2026-04-18
> **Status:** ✅ COMPLETED (structural refactor) — polish work deferred to `plan-client-ui-polish.md`
> **Completed:** 2026-04-18. WPs 0–9 landed and pushed to main. All 31 locked decisions (D1–D31) honored at the WIT + trait + host-component level. `cargo check --workspace` clean; `cargo test -p poly-core --lib` 84 passed; `cargo test -p poly-lint-gate --lib` 40 passed; `cargo test -p poly-chat-mcp` 48 passed. User's original bug (Lemmy community showing Discord-only menu items) fixed; verified live on poly-web at `/demo_forum/demo_forum/demo-platypus/channels/comm-rust-lang`.
> **What shipped:** the 6 new WIT interfaces (`client-ui-common`, `client-menus`, `client-settings`, `client-sidebar`, `client-views`, `client-composer`), the `ClientBackend` trait migration with no-default methods across all 10 backends, the 6 host components (`ClientMenu`, `PluginSettingsSection`, `ClientSidebar`, `ClientView`, `ComposerHooks`/`MessageActions`, `CustomBlock`), the 2 new lint scanners (FTL label-key coverage + action-id kebab-case naming), the forbid-slug-match-in-UI scanner, 18 new MCP tools, ammonia-based HTML sanitizer with 15 security tests, and 3 documentation files (`plugin-authoring.md`, `plugin-testing.md`, `client-ui-test-matrix.md`). Per-backend Rust UI files deleted per D7. `backend-type` WIT enum removed per D17. Zero slug-match sites remain in `crates/core/src/ui/`.
> **What's deferred to `plan-client-ui-polish.md`:** 61 follow-up items covering visible behavior gaps (plugin settings storage stubbed, `ActionOutcome::Navigate/Toast/Pending` logged but not actioned, row clicks no-op, `SidebarInvalidated` event not consumed, `get_view_rows` empty for 4 of 5 forum backends), placeholder renderings (SpacesRooms/Communities/Feed/RepoTree layouts minimal, icons passed-through-but-not-rendered), cosmetic polish (no hover states, no separators, no ARIA, no keyboard nav), and cleanup debt (`BackendCapabilities` D12 flags pending removal, `backend_emoji` survivor slug match, dead `forum_view.rs` helpers). The polish plan preserves all structural guarantees of this plan while refining UX to production quality.
...
> **Execution model:** All work packages were executed by AI coding agents in a single integrated pass. Work-package structure exists solely to partition the graph for scheduling parallel agents and to define integration milestones — not as time estimates.
